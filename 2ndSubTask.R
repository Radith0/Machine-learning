...
data<-read_excel(path = "ExchangeUSD.xlsx") 
...
lag_data <- list()
lag_data[[1]] <- data %>% mutate(t.1 = lag(rate, 1)) %>% na.omit()
lag_data[[2]] <- data %>% mutate(t.1 = lag(rate, 1), t.2 = lag(rate, 2)) %>% na.omit()
lag_data[[3]] <- data %>% mutate(t.1 = lag(rate, 1), t.2 = lag(rate, 2), t.3 = lag(rate, 3)) %>% na.omit()
lag_data[[4]] <- data %>% mutate(t.1 = lag(rate, 1), t.2 = lag(rate, 2), t.3 = lag(rate, 3), t.4 = lag(rate, 4)) %>% na.omit()
...
#c
# Defining a function to train neural network models
...
# List to store trained neural networks
NN<-list() 

# Looping through different configurations to train models 
...
#f
# Extracting RMSE values and output types for plotting
...
ggplot(rmse_data, aes(x = model, y = rmse, fill = output_type)) + 
  ...
  scale_fill_manual(values = c("Linear" = "green", "Non-Linear" = "orange")) + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1))  

# Extracting MAE values and output types 
mae_data <- data.frame( 
  model = names(evaluations), 
...
# Extracting MAPE values and output types 
...
# Extracting SMAPE values and output types 
...
rmse_plot <- plot_performance(rmse_data, "rmse", "RMSE Performance of Linear MLP 
Models") 
mae_data <- data.frame( 
  model = names(linear_evaluations), 
...
mape_plot <- plot_performance(mape_data, "mape", "MAPE Performance of Linear MLP 
Models") 
smape_data <- data.frame( 
  model = names(linear_evaluations), 
...
smape_plot <- plot_performance(smape_data, "smape", "SMAPE Performance of Linear MLP 
Models") 
...
# Extracting top 5 models for each metric 
...
# Extracting the test data for lag 1 
lag_num <- 1 
test_data <- lag_test_data[[lag_num]] 
...
ggplot(data_to_plot, aes(x = seq_along(Actual), y = Actual)) + 
  geom_line(color = "lightblue", linetype = "dashed") + 
  geom_line(aes(y = Predictions), color = "yellow") + 
